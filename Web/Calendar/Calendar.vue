<template>
  <div>
    <div class="tk-g-root-container tk-layout-flex-row">
      <div class="tk-layout-full-height tk-layout-flex-1 tk-layout-flex-column">
        <!-- operations -->
        <div
        ...
        >
          <!-- title -->
          <div class="tk-px-width-px120 tk-px-margin-right-px32 tk-page-title">
            {{ $t("calendar.lesson") }}
          </div>
          ...
          >
            ...
            <template v-if="!showMonitoring">
              ...
              <base-button
                ...
                @click.prevent="prev"
              >
                <i class="fas fa-angle-left tk-position-center"></i>
              </base-button>
              ...
              <base-button
                ...
                @click.prevent="next"
              >
                <i class="fas fa-angle-right tk-position-center"></i>
              </base-button>
            </template>
          </div>
          ...
          >
            <template
              v-for="(item, index) in $commons.mapToArray(filterUserMap)"
            >
              <avatar
                v-if="index < 3"
            ...
            <span v-if="Object.keys(filterUserMap).length >= 3">...</span>
          </div>
        </div>
        ...
        >
              ...
              <div
                v-if="
                  ($commons.userIsInstructor(userInfo) ||
                    (!$commons.userIsInstructor(userInfo) && item.type == 0)) &&
                  item
                "
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <span @click.stop="archiveAllCanceledLesson"
                      >Archive All</span
                    >
                  </div>
                </div>
                <calendar-monitoring
                  ...
                  :extraOptions="{
                    ...
                    studentsMap: $commons.userIsInstructor(userInfo)
                      ? studentsListMap
                      : kidsUserInfoMap,
                    userInfo: userInfo,
                  ...
                  @onClickStudentCancelNewLesson="onClickStudentCancelNewLesson"
                  id="tkMonitor"
                ></calendar-monitoring>
              </div>
              ...
              <div
                ...
                v-if="
                  ...
                  !$tools.objIsNull(lessonDetailForMonitorItem)
                "
              >
                <!-- lesson detail under monitor -->
                <div class="tk-px-margin-bottom-px16">
                  <div class="tk-px-height-px36 tk-font-color-gray">
                    {{ $t("calendar.lesson_detail") }}
                  </div>

                  <lesson-detail-card
                    v-if="!$tools.isNull(lessonDetailForMonitorItem)"
                    ...
                    :teacher="teachersMap[lessonDetailForMonitorItem.teacherId]"
                    :student="
                      !$tools.isNull(lessonDetailForMonitorItem.studentId)
                        ? $commons.userIsInstructor(userInfo)
                          ? studentsListMap[
                    ...
                    "
                    :lessonType="
                      lessonTypesMap[lessonDetailForMonitorItem.lessonTypeId]
                    "
                    :lessonConfig="
                      lessonConfigsMap[
                        lessonDetailForMonitorItem.lessonScheduleConfigId
                      ]
                    "
                    :instrumentUrl="
                      !isNull(
                      ...
                      !isNull(
                        instruments[
                          lessonTypesMap[
                            lessonDetailForMonitorItem.lessonTypeId
                          ].instrumentId
                        ]
                      )
                        ? instruments[
                ...
                <div class="tk-layout-flex-1">
                  ...
                  <conversation-card
                    ...
                    style="max-height: 360px; min-height: 160px"
                  ></conversation-card>
                </div>
              </div>
              ...
              <div
                v-if="
                  !showLessonDetailForMonitorItem && getMonitorData().length > 0
                "
                class="tk-overflow-y tk-layout-full-height tk-scrollbar tk-layout-flex-1"
              >
                <div
          ...
          <div
            ...
            :class="{
              'tk-bg-gradient-7 tk-font-color-black2': !selectedTime.isPast,
              'tk-bg-gradient-8 tk-font-color-gray': selectedTime.isPast,
            }"
            ...
            "
            :style="{
              paddingLeft: defaultView == viewType.week ? '0.5rem' : '1.25rem',
              fontSize: defaultView == viewType.week ? '0.75rem' : '0.875rem',
            }"
        ...
        <slide-x-right-transition :duration="300" mode="out-in">
          ...
          >
            <detail-in-calendar
              :userId="
                (showMonitoring
                  ? lessonDetailForMonitorItem.studentId
                  : lessonDetail.studentId) || previewStudentId
              "
          ...
          >
            <lesson-detail
              ...
              :lessonType="lessonTypesMap[lessonDetail.lessonTypeId]"
              :config="lessonConfigsMap[lessonDetail.lessonScheduleConfigId]"
    ...
    <lesson-detail-card
      ...
      :style="{
        minHeight:
          lessonDetail.shouldDateTime && lessonDetail.lessonScheduleConfigId
            ? '240px'
            : '80px',
      }"
      :data="lessonDetail"
      :teacher="
        lessonDetail.teacherId ? teachersMap[lessonDetail.teacherId] : {}
      "
      :student="
      ...
      :instrumentUrl="
        ...
        !isNull(
          instruments[lessonTypesMap[lessonDetail.lessonTypeId].instrumentId]
        )
          ? instruments[lessonTypesMap[lessonDetail.lessonTypeId].instrumentId]
              .minPictureUrl
          : ''
      ...
      :extraOptions="{
        timestamp:
          (lessonDetail.shouldDateTime || lessonDetail.startTime) * 1000,
        displayKidsForParent:
          $commons.userIsParent(userInfo) && lessonDetail.lessonScheduleConfigId
    ...
    <alert
      :title="alert.title"
      :content="alert.isNoShow || alert.isCancelation ? '' : alert.content"
      :left="alert.left"
      :right="alert.right"
...
export default {
  ...
  components: {
    ...
    FutureLessons: () => import("@/tkViews/components/Modal/FutureLessons.vue"),
  ...
  async mounted() {
    ...
    self.$bus.$on("onChangeLessonSchedule", async (data) => {
      if (self.enableListener) {
        ...
        if (
          data.cancelled ||
          (data?.rescheduled && !self.$tools.isNull(data.rescheduleId))
        ) {
    ...
    self.$bus.$on("onAddLessonFollowUp", async (data) => {
      if (self.enableListener) {
        ...
        if (
          data.column == self.$dataModules.followUp.column.unconfirmed &&
          data.dataType == self.$dataModules.followUp.dataType.reschedule &&
          !self.$tools.isNull(data?.data?.timeBefore)
        ) {
          let schedule = await self.$scheduleService.lessonScheduleAction.get(
      ...
      handler(newVal) {
        ...
        if (!this.isNull(newVal.ymd) && !this.isNull(newVal.timeStr)) {
          ...
          if (ymd.indexOf("+") > -1) {
            ...
            self.selectedTime.isPast = timestamp < self.$moment().unix();
          }
        }
      },
    },
  ...
  computed: {
    lessonScheduleConfig() {
      ...
      if (self.showReschedule) {
        ...
        return self.lessonConfigsMap[lessonDetail.lessonScheduleConfigId];
      }
    },
    hasFilter() {
      let self = this;
  ...
  methods: {
    async reRenderEvent(data, isCache) {
      ...
      if (!isCache) {
        ...
        for (let key in schedulesMap) {
          ...
          if (
            schedule.shouldDateTime <
              self.$moment(self.startTimestamp).subtract(7, "days").unix() ||
            schedule.shouldDateTime >
              self.$moment(self.endTimestamp).add(7, "days").unix()
          ) {
            continue;
          }
          ...
          if (schedule.shouldDateTime > self.$moment().unix() && item) {
            console.log(
              "init item: ",
          ...
          if (
            ...
            console.log("delete cancelled || rescheduled: ", schedule);
        ...
        Object.keys(self.lessonSchedulesMap).forEach((key) => {
          let schedule = self.lessonSchedulesMap[key]?.extendedProps?.data;
          if (
            !schedulesMap[key] &&
            schedule.shouldDateTime >=
              self.$moment().startOf("month").subtract(7, "days").unix() &&
            schedule.shouldDateTime <=
              self.$moment(self.endTimestamp).add(7, "days").unix() &&
        ...
        self.getFollowUpUnconfirmedEvents(self.lessonUnconfirmedMap ?? {});
    ...
    async clearCacheAndReload(onlineConfig = true) {
      ...
      self.lessonConfigsMap = await self.$scheduleService.lessonScheduleConfig(
        onlineConfig
      );
    ...
    async initCache() {
      ...
      // business hour cache
      let businessHoursCache = await self.$tools.getCache("temp_businessHours");
      ...
      self.options.events = self.$commons.mapToArray(self.lessonSchedulesMap);
      ...
      // monitor cache
      let tempUnconfirmedCache = await self.$tools.getCache("temp_unconfirmed");
      if (!self.isNull(tempUnconfirmedCache)) {
        ...
        self.canceled = (await self.$tools.getCache("temp_canceled")) ?? 0;
        self.noShows = (await self.$tools.getCache("temp_noShows")) ?? 0;
      }
    ...
    async initBasicData(now) {
      ...
      if (self.$commons.userIsInstructor(self.userInfo)) {
        ...
        self.$tools.setCache("ur_cr_cr_pe", self.userCalendarColorPreference);
        console.log(
          "userCalendarColorPreference: ",
          self.userCalendarColorPreference
        );
        ...
        let onlineLinks = self.teacherInfo?.addressBooks?.filter(
          (item) =>
            item.type == self.$dataModules.lessonSchedule.locationType.remote
        );
        ...
        self.dataVersion = await self.$userService.dataVersionAction.get(
          self.userInfo.userId
        );
        ...
        console.log("googleCalendarSyncStatus", self.googleCalendarSyncStatus);
        console.log(
          "googleCalendarSyncRequest",
          self.googleCalendarSyncRequest
        );
        ...
        if (
          !self.$tools.isNull(self.googleAccessTokenCalendar.refresh_token) &&
          self.googleAccessTokenCalendar?.expiry_date <= new Date().getTime()
        ) {
    ...
    async clearLessonsUnderConfig(config, callback) {
      ...
      let lessonSchedules = self.$commons
        ...
        ?.filter((item) => {
          return item.extendedProps?.data?.lessonScheduleConfigId == config.id;
        });
      console.log("清除 config 下的 lesson: ", config);

      for (let i = 0; i < lessonSchedules.length; i++) {
        let item = lessonSchedules[i];
        let shouldDateTime = self.lessonSchedulesMap[item?.id]?.shouldDateTime;
        let needClear =
        ...
        console.log(
          ...
          new Date(self.lessonSchedulesMap[item?.id]?.shouldDateTime * 1000)
        );
      ...
      self.$tools.setCache("temp_lessonSchedulesMap", self.lessonSchedulesMap);
    ...
    async updateCalendarEventByConfig(config) {
      ...
      let lessons = await self.getLessonsByConfig(self.startTimestamp, config);
    ...
    async initLessonDetailForMonitorItem(monitorItem) {
      ...
      if (data?.data?.credit) {
        ...
        let lessonScheduleId = self.getLessonScheduleIdInMonitorItem(data);

        lessonDetail = await self.$scheduleService.lessonScheduleAction.get(
          lessonScheduleId
        );
      }
      ...
      self.lessonDetailForMonitorItem =
        ...
        self.$dataModules.followUp.dataType.studentLessonConfigRequests
          ? {
              ...
              shouldTimeLength:
                self.lessonTypesMap[data?.data?.config?.lessonTypeId]
                  ?.timeLength ?? 0,
      ...
      if (data.data.data) {
        data.data.data?.some((item) => {
          self.timestampForMonitorItem.push(item?.shouldDateTime * 1000);
        });
      } else {
    ...
    async cancelLesson(data) {
      ...
      let cancelLesson = data
        ...
        : config?.lessonCategory == self.$dataModules.lessonType.category.group
    ...
    async archiveAllCanceledLesson() {
      let self = this;
      let now = self.$moment().unix();
      let archiveAllFollowUps = self.$functionsService.archiveAllFollowUps;
      ...
      let unArchivedCancellations = self.lessonCanceled?.filter(
        (item) => item.status != self.$dataModules.followUp.status.archived
      );
      let ids = [];
    ...
    async archiveCanceledLesson(id) {
      ...
      let archiveCancellation = self.$functionsService.archiveCancellation;
    ...
    confirmReschedule(data) {
      ...
      switch (data.dataType) {
        ...
        case self.$dataModules.followUp.dataType.newLessonFromCredit:
          confirmReschedule = self.$functionsService.confirmNewLessonFromCredit;
          break;
      }
    ...
    addCredit(data) {
      ...
      let addCredit =
        data.dataType == self.$dataModules.followUp.dataType.cancellation
    ...
    declineReschedule(data) {
      ...
      switch (data.dataType) {
        case self.$dataModules.followUp.dataType.reschedule:
          declineReschedule = self.$functionsService.declineReschedule;
          break;
        case self.$dataModules.followUp.dataType.newLessonFromCredit:
          declineReschedule = self.$functionsService.declineNewLessonFromCredit;
          break;
      }
      ...
      if (declineReschedule) {
        declineReschedule({
          ...
          .catch(async (err) => {
            ...
            self.$bus.$emit("hideFullCover", {
              message: self.$i18n.t("notification.failed.decilne"),
              type: "error",
              unix: now,
            });
          });
    ...
    onUpdateStudentsForGroupLesson(data) {
      ...
      updateStudentsForGroupLesson({
        ...
        .then((res) => {
          console.log("updateStudentsForGroupLesson 成功: ", res.data);
          self.$bus.$emit("hideFullCover", {
            message: self.$i18n.t("notification.success.update"),
            type: "success",
            unix: now,
    ...
    async onClickShare(data) {
      let self = this;
      let now = self.$moment().unix();
      let linkCache = (await self.$tools.getCache("temp_lessonLinksMap")) ?? {};
        ...
        self.$axios
          ...
          .then(async (res) => {
            if (res?.data?.shortLink) {
              ...
              linkCache[data?.lessonScheduleConfigId] = res.data.shortLink;
              self.$tools.setCache("temp_lessonLinksMap", linkCache);

              self.$bus.$emit("hideFullCover", {
                message: self.$i18n.t("notification.success.share"),
                type: "success",
                unix: now,
              });
            }
    ...
    onClickArchive(data) {
      ...
      self.alert.show = true;
      self.alert.onLeftTap = () => {
        if (data.dataType == self.$dataModules.followUp.dataType.cancellation) {
    ...
    onClickRetract(data) {
      ...
      self.alert.content = self.$i18n.t("alert.retract_reschedule_lesson");
    ...
    async onClickChange(data) {
      ...
      self.selectedTime = {
        ...
        timeStr:
          data?.data?.timeAfter > 0
            ? self.$moment(data?.data?.timeAfter * 1000).format("h:mmA")
            : self
                .$moment(self.lessonDetail?.shouldDateTime * 1000)
                .format("h:mmA"),
        timestamp:
          data?.data?.timeAfter > 0
            ? data?.data?.timeAfter
        ...
        ymd:
          data?.data?.timeAfter > 0
            ? self.$moment(data?.data?.timeAfter * 1000).format("YYYY/M/D")
            : self
                .$moment(self.lessonDetail.shouldDateTime * 1000)
                .format("YYYY/M/D"),
      };
    ...
    async onClickReschedule(data) {
      let self = this;
      let now = self.$moment().unix();
      self.lessonDetail = await self.$scheduleService.lessonScheduleAction.get(
        data.data.scheduleId
      );
    ...
    async onClickIgnore(data) {
      ...
      self.alert.content = "Are you sure you want to ignore the conflict?";
    ...
    async monitorOnChanged(
      ...
      if (option?.isChange) {
        ...
        self.followUpMap[data.id].lessonConfig =
          self.lessonConfigsMap[lessonSchedule?.lessonScheduleConfigId];
      }

      switch (data.column) {
        case self.$dataModules.followUp.column.unconfirmed:
          if (!option?.isRemove) {
            self.lessonUnconfirmedMap[data.id] = self.$tools.equalValue(
              self.followUpMap[data.id]
            );
            if (data?.data?.oldScheduleId) {
              await self.removeCalendarEvent(data.data.oldScheduleId, true);
            }
        ...
        case self.$dataModules.followUp.column.cancelled:
          if (!option?.isRemove) {
            self.lessonCanceledMap[data.id] = self.$tools.equalValue(
              self.followUpMap[data.id]
            );
            if (data.data?.id) {
        ...
        case self.$dataModules.followUp.column.rescheduled:
          if (!option?.isRemove) {
            ...
            if (data?.id) {
              ...
              await self.removeCalendarEvent(data?.data?.newSchedule?.id, true);
              await self.addCalendarEvent(data?.data?.newSchedule, true);
            }
          }

          if (option?.isChange) {
            console.log("非 change rescheduled data: ", data, data.id);
            self.updateFollowUpArray(
              self.lessonRescheduledMap,
              "lessonRescheduledMap",
              "rescheduled"
                ...
                (item) =>
                  (!item?.data?.teacherRead &&
                    ...
                    self.userInfo?.roleIds?.indexOf(
                      self.$dataModules.user.roleId.instructor
                    ) > -1) ||
                  (!item?.data?.studioManagerRead &&
                    self.userInfo?.roleIds?.indexOf(
                      self.$dataModules.user.roleId.studioManager
                    ) > -1)
              ).length
          : self.$commons
              .mapToArray(self[mapName])
              .filter(
                (item) =>
                  item.status !== self.$dataModules.followUp.status.ignored &&
                  item.status !== self.$dataModules.followUp.status.archived &&
                  item.status !== self.$dataModules.followUp.status.read
              )?.length ?? 0;
    ...
    async initPending() {
      ...
      let lessonUnconfirmedMap = await self.$scheduleService.lessonFollowUp({
    ...
    async initUnconfirmed() {
      ...
      Object.keys(lessonUnconfirmedMap).forEach(async (key) => {
        ...
        let lessonScheduleId = self.getLessonScheduleIdInMonitorItem(followUp);
        getLessonSchedule.push(
          self.$scheduleService.lessonScheduleAction.get(lessonScheduleId)
        );
      });
      ...
      Object.keys(lessonUnconfirmedMap).forEach((key) => {
        ...
        lessonUnconfirmedMap[key].lessonConfig =
          self.lessonConfigsMap[
            lessonSchedulesMap[lessonScheduleId]?.lessonScheduleConfigId
          ];
    ...
    async initRescheduled(unix) {
      let self = this;
      let lessonRescheduledMap = await self.$scheduleService.lessonFollowUp({
      ...
      if (!self.$tools.isNull(lessonRescheduledMap)) {
        Object.keys(lessonRescheduledMap).forEach((key) => {
          ...
          lessonRescheduledMap[key].lessonScheduleId = lessonScheduleId;
        });
    ...
    async initNoShows() {
      ...
      self.updateFollowUpArray(lessonNoShowsMap, "lessonNoShowsMap", "noShows");
    },
    // 首字母大写
    ...
    initOptions() {
      ...
      self
        .calendarApi()
        .setOption("eventClassNames", "tk-cursor-pointer " + self.defaultView);
      ...
      self.calendarApi().setOption(
        ...
        self.defaultView == self.viewType.month
          ? { weekday: "short" }
          : {
              weekday: self.defaultView == self.viewType.day ? "long" : "short",
              month: "numeric",
              day: "numeric",
              omitCommas: true,
            }
      );
      self.calendarApi().setOption("slotMinTime", self.businessHours.startTime);
      self.calendarApi().setOption("slotMaxTime", self.businessHours.endTime);
    ...
    setCalendarHeight() {
      ...
      this.calendarApi().setOption("contentHeight", $(window).height() - 100);
    ...
    getSelectTime(data, callback) {
      ...
      if (data?.shouldDateTime) {
        self.selectedTime.isPast = data.shouldDateTime < self.$moment().unix();
    ...
    scrollOnDetailTime(ev) {
      let self = this,
        e = ev || window.event;
      let scrollTop =
        $(".fc-scroller.fc-scroller-liquid-absolute")[0].scrollTop + e.deltaY;
        ...
        if (self.defaultView !== self.viewType.month) {
          ...
          if (
            ...
            if (
              ...
              target.firstElementChild.className.indexOf("fc-scroller") > 0
            ) {
              isTimeGrid = false;
            }
            ...
            if (isTimeGrid) {
              let width = 0,
                axisW = $(".fc-timegrid-slot-label").eq(0).width();
              ...
              let firstViewDayTime = !self.isNull(self.currentViewDays[0])
              ...
              if (self.defaultView == self.viewType.week) {
                ...
                if (offsetX >= baseLeft && offsetX <= baseLeft + baseWidth) {
                  // left = baseLeft + 4;
                  left = baseLeft;
                  ...
                  left = baseLeft + width;
                  self.selectedTime.ymd =
                    self.$commons.formatYMD(firstViewDayTime) + "+1";
                } else if (
                  offsetX > baseLeft + baseWidth * 2 &&
                  ...
                  self.selectedTime.ymd =
                    self.$commons.formatYMD(firstViewDayTime) + "+2";
                } else if (
                  offsetX > baseLeft + baseWidth * 3 &&
                  ...
                  self.selectedTime.ymd =
                    self.$commons.formatYMD(firstViewDayTime) + "+3";
                } else if (
                  offsetX > baseLeft + baseWidth * 4 &&
                  ...
                  self.selectedTime.ymd =
                    self.$commons.formatYMD(firstViewDayTime) + "+4";
                } else if (
                  offsetX > baseLeft + baseWidth * 5 &&
              ...
              if (!self.isNull(hhmm)) {
                ...
                if (parseInt(timeStrMin) >= 0 && parseInt(timeStrMin) < 15) {
                  timeStrMin = "00";
                } else if (
              ...
              let isCoverEvent = await self.detailTimeCoverEvent(top, left);
    ...
    detailTimeCoverEvent(top, left) {
      ...
      for (let i = 0; i < els.length; i++) {
        ...
        if (
          (topMinus <= 16 || (top >= itemTop && top <= itemTop + height)) &&
          leftMinus <= 16
        ) {
          isCover = true;
          break;
          ...
          self.$commons
            ...
            ?.filter(
              (item) =>
                ...
                (
                  item.visibility ==
                    self.$dataModules.lessonType.visibility.students ||
                  item.visibility !=
                    self.$dataModules.lessonType.visibility.anyone ||
    ...
    confirmFilter(data) {
      ...
      if (data.studioRoomsMap) {
        self.filterRoomMap = self.$tools.equalValue(data.studioRoomsMap);
      }

      if (data.locationsMap) {
        self.filterOtherLocationMap = self.$tools.equalValue(data.locationsMap);
      }

      if (data.lessonTypesMap) {
        self.filterLessonTypeMap = self.$tools.equalValue(data.lessonTypesMap);
      }
    ...
    meetFilter(item) {
        ...
        meetLocation =
          ...
          (self.filterOtherLocationMap["OTHER_PLACE"] &&
            (self.$commons.locationIsOtherPlace(item?.location) ||
              self.$commons.locationIsStudioLocation(item?.location)));
      }
      ...
      if (self.$tools.objIsNull(self.filterTagMap)) {
        ...
        meetTag =
          ...
          student?.tags?.some((tag) =>
            tag?.tags?.some(
              (t) => !self.isNull(self.filterTagMap[tag.category + "__" + t])
            )
          );
      }
      ...
      return meetUser && meetLessonType && meetRoom && meetLocation && meetTag;
    },
    // 打开当前登录用户的 profile
    showProfile() {
    ...
    async onEventRender(arg) {
      ...
      if (
        ...
        console.log("render rescheduled event: ", data, timeUnix, timeStr);
    ...
    async initLessonDetailForEvent(event) {
      ...
      if (data?.shouldDateTime) {
        console.log(
          ...
          self.lessonConfigsMap[data.lessonScheduleConfigId],
          new Date(
            self.lessonConfigsMap[data.lessonScheduleConfigId]?.startDateTime *
              1000
          ),
          new Date(
            self.lessonConfigsMap[data.lessonScheduleConfigId]?.endDate * 1000
          )
        );
    ...
    initLessonDetailPosition(ele, jsEvent) {
      ...
      if (x + elW > (winW / 3) * 2 && self.defaultView != self.viewType.day) {
        // 左
    ...
    async onEventClick({ event, el, jsEvent, view }) {
      ...
      if (
        ...
        await self.initLessonDetailPosition(
          ele,
          !self.$tools.isNull(evt?.extendedProps?.type) ? jsEvent : false
        );
    ...
    async getStudioEvents(map) {
      ...
      let colorPreference = self.userCalendarColorPreference?.items?.filter(
        (item) =>
          item.category ==
          self.$dataModules.userCalendarColorPreference.category.studioEvents
      )[0];
      let colorsMap = {};
        ...
        let displayEndTime =
          item.endTime > 0
            ? self
                ...
                .startOf("date")
                .unix()
            : self.$moment().add(1000, "years").endOf("date").unix();
    ...
    async getGoogleEvents(map, showLoading = false) {
      ...
      if (
        ...
        self.googleCalendarSyncRequest?.syncFromGoogleCalendars?.length < 1
      ) {
        if (showLoading) {
          self.$bus.$emit("hideFullCover", {
            type: "success",
      ...
      if (self.$tools.isNull(self.googleAccessTokenCalendar.access_token)) {
        self.googleAccessTokenCalendar =
          (await self.$userService.googleAccessTokenCalendarAction.get(
            self.userInfo?.userId
          )) ?? {};
      ...
      if (map) {
        ...
        eventsArray?.some((item) => {
          if (activeGoogleCalendarsMap[item.calendarId]) {
            activeGoogleCalendarsMap[item.calendarId].events = item.events;
          }
        });

        Object.keys(activeGoogleCalendarsMap)?.forEach((key) => {
          let calendar = activeGoogleCalendarsMap[key];
          let backgroundColor = calendar?.calendarData?.backgroundColor;
            ...
            calendar.events.some((event) => {
              if (event?.status != "cancelled") {
                let data = {
                  ...
                  allDay: !self.$tools.isNull(event?.start?.date),
                  originData: event,
                };
                ...
                if (!self.$tools.isNull(event?.start?.date)) {
                  ...
                  startTime = self.$momentTimezone
                    .tz(`${event.start.date} 00:00`, data.timeZone)
                    .unix();
                  endTime = self.$momentTimezone
                    .tz(`${event.end.date} 00:00`, data.timeZone)
                    .unix();
                } else if (!self.$tools.isNull(event?.start?.dateTime)) {
                  // time event
                  startTime = self.$momentTimezone
                    .tz(event?.start?.dateTime, data.timeZone)
                    .unix();
                  endTime = self.$momentTimezone
                ...
                if (event?.recurrence?.length > 0 && false) {
                  ...
                  let startDTSTART = `DTSTART;TZID=${data.timeZone}:${self
                  ...
                  let byDayStr = event?.recurrence
                    ...
                    ?.split("BYDAY=")[1]
                    ?.split(";")[0];
                  let pos = byDayStr?.substring(0, byDayStr.length - 2);
                  let setpos = self.$tools.isNull(pos) ? null : pos;
                  let startByweekdays = localStartRRule?.origOptions?.byweekday;
                  let startWkst = localStartRRule?.origOptions?.wkst;
                  ...
                  if (startByweekdays) {
                    startByweekdays?.some((item, index) => {
                      startByweekdays[index] = self.$commons.getUTCRRuleWeekday(
                        item?.weekday + 1 > 6 ? 0 : item?.weekday + 1,
                        startTime
                      );
                    });
                  ...
                  if (startWkst) {
                    startWkst = self.$commons.getUTCRRuleWeekday(
                      startWkst?.weekday + 1 > 6 ? 0 : startWkst?.weekday + 1,
                      startTime
                    );
                  ...
                  startTimeArr.some((item, index) => {
                    let dataItem = {
                      ...data,
                      startTime: self.$moment(item).unix(),
                      endTime: self
                        .$moment(item)
                        .add(endTime - startTime, "seconds")
                        .unix(),
                    };
    ...
    async getFollowUpUnconfirmedEvents(map) {
      ...
      for (let key in self.lessonUnconfirmedMap) {
        ...
        if (
          ...
          let item = await self.$scheduleService.lessonScheduleAction.get(
            scheduleId
          );
          ...
          if (item?.id) {
            ...
            if (!self.isNull(config)) {
                      ...
                      .filter(
                        (i) =>
                          (i.status ==
                            ...
                            i.registrationTimestamp <= item.shouldDateTime) ||
                          (i.status ==
                            self.$dataModules.lessonScheduleConfig
                              .groupLessonStudentsStatus.quit &&
                            i.registrationTimestamp <= item.shouldDateTime &&
                            i.quitTimestamp > item.shouldDateTime)
                      )?.length + " students"
                  : self.studentsListMap[item.studentId]?.name ?? "", // name
                classNames: classNamesBasic.concat(specificClassName),
                start: displayTime * 1000,
                end: (displayTime + item.shouldTimeLength * 60) * 1000,
                display: "block",
              ...
              if (!self.$tools.isNull(eventItem?.id)) {
                let event = self.calendarApi()?.getEventById(eventItem?.id);
                ...
                console.log("add unconfirmed event: ", eventItem);
              }
            }
          }
        }
    ...
    getLessonColor(item) {
      ...
      if (hasLocation) {
        ...
        let studioRoom =
          self.studioRoomsMap[item?.location?.id || item?.location?.place];
        ...
        if (
          ...
          let sameNameAddress =
            self.teacherInfo?.addressBooks?.filter(
              (addr) => !addr.isDelete && addr?.title == studioRoom?.name
            )[0] ?? null;
          if (studioRoom?.isDeleted && !self.$tools.isNull(sameNameAddress)) {
        ...
        if (
          ...
          let sameNameStudioRoom =
            self.$commons
              ...
              ?.filter(
                (room) =>
                  !room.isDeleted && room?.name == (item?.title || item?.name)
              )[0] ?? null;

          if (address?.isDelete && !self.$tools.isNull(sameNameStudioRoom)) {
            item.location = {
              id: sameNameStudioRoom?.id,
              place: sameNameStudioRoom?.id,
              type: self.$dataModules.lessonSchedule.locationType.studioRoom,
      ...
      bgColor =
        ...
        bgColorsMap?.[self.onlineLinksMap?.[item?.location?.remoteLink]?.id]
          ?.block ||
        ...
        (!self.$commons.instructorIsManagerInStudio(
          ...
          : "") ||
        (!self.$tools.isNull(blockColorTag) && blockColorTag?.tags?.length > 0
      ...
      borderColor =
        ...
        borderColorsMap?.[self.onlineLinksMap?.[item?.location?.remoteLink]?.id]
          ?.line ||
        ...
        (!self.$commons.instructorIsManagerInStudio(
          ...
          : "") ||
        (!self.$tools.isNull(lineColorTag) && lineColorTag?.tags?.length > 0
    ...
    async getLessons(timestamp, events = {}, clearAll = false) {
      ...
      if (self.$commons.userIsInstructor(self.userInfo)) {
        if (events?.readStudioEvents) {
          studioEventsMap =
            (await self.getStudioEvents(self.studioEventsMap ?? {})) || {};
        } else {
      ...
      if (timestamp) {
        ...
        if (self.defaultView != self.viewType.month) {
          ...
          Object.keys(self.lessonSchedulesMap).forEach((key) => {
            ...
            if (
              item?.extendedProps?.data?.shouldDateTime >= startMoment.unix() &&
              item?.extendedProps?.data?.shouldDateTime <= endMoment.unix()
            ) {
              self.removeCalendarEvent(key);
            }
          });
        ...
        if (self.defaultView == self.viewType.month) {
          ...
          console.log("刷新events: ", Object.keys(eventsMap ?? {})?.length);

          let lessons = self.$commons
            .mapToArray(lessonSchedulesMap)
            ?.filter((item) => self.meetFilter(item?.extendedProps?.data));
    ...
    async getLessonsByConfig(timestamp, config) {
      return new Promise(async (resolve, reject) => {
        ...
        let schedules = await this.$scheduleService.calculateScheduleByConfig({
          timestamp: timestamp,
          startTime: self
            .$moment(self.calendarApi()?.view.activeStart)
            .startOf("date")
        ...
        for (let id in schedules) {
          ...
          if (
            ...
            (self.$commons.userIsParent(self.userInfo) &&
              self.userInfo?.kids?.indexOf(item.studentId) > -1) ||
            (self.$commons.userIsParent(self.userInfo) &&
            ...
            (self.$commons.userIsStudent(self.userInfo) &&
              self.lessonConfigsMap[item.lessonScheduleConfigId]
                ...
                self.$dataModules.lessonType.category.group &&
              self.lessonConfigsMap[item.lessonScheduleConfigId]
                ?.groupLessonStudents[self.userInfo?.userId]?.status ==
                self.$dataModules.lessonScheduleConfig.groupLessonStudentsStatus
                  .active)
          ) {
            if (
              self.lessonConfigsMap[item.lessonScheduleConfigId]?.endType ==
                1 &&
              item?.shouldDateTime >
                self.lessonConfigsMap[item.lessonScheduleConfigId]?.endDate
            ) {
              console.log(
                "排除 endType == 1: ",
                self.lessonConfigsMap[item.lessonScheduleConfigId]
              ...
              getInitEventItem.push(self.initEventItem(schedules[id]));
            }
          }
        }
    ...
    async initEventItem(data) {
      ...
      if (
        (config?.repeatType !=
          ...
          config?.endType ==
            self.$dataModules.lessonScheduleConfig.endType.endAtSomeday &&
      ...
      if (!self.isNull(config)) {
        let displayTime = self.$commons.getLessonShouldDateTime(item, config);
            ...
            title:
              ...
              (self.$commons.userIsInstructor(self.userInfo)
                ? self.$tools.isNull(item.studentId)
                  ...
                  : self.studentsListMap[item.studentId]?.name ?? ""
                : self.$commons.userIsStudent(self.userInfo)
                ? self.lessonTypesMap[item.lessonTypeId]?.name ?? ""
                : self.formatKidsInLesson(item).name), // name
            classNames: classNamesBasic.concat(specificClassName),
            start: displayTime * 1000,
            end: displayTime * 1000 + item.shouldTimeLength * 60 * 1000,
            display: "block",
            backgroundColor: `#${bgColor}`,
            borderColor: `#${borderColor}`,
            extendedProps: {
    ...
    getActiveGroupLessonStudents(groupLessonStudentsMap) {
      ...
      Object.keys(groupLessonStudentsMap).forEach((key) => {
        ...
        if (
          item.status ==
          self.$dataModules.lessonScheduleConfig.groupLessonStudentsStatus.quit
        ) {
          delete data[key];
        }
      });
    ...
    formatKidsInLesson(lessonData) {
      let self = this;
      let config = self.lessonConfigsMap[lessonData.lessonScheduleConfigId];
      let result = {
        name: "",
        studentId: "",
        array: [],
      ...
      if (
        ...
        // single
        result.name = self.kidsUserInfoMap[lessonData?.studentId]?.name ?? "";
        result.studentId = lessonData?.studentId;
        result.email = self.kidsUserInfoMap[lessonData?.studentId]?.email ?? "";
      } else if (
        config?.lessonCategory == self.$dataModules.lessonType.category.group
      ) {
        // group
    ...
    async initTitle(option) {
      ...
      let startTimestamp =
        self.$moment(currentStartTimestamp).startOf("date").unix() * 1000;
      let endTimestamp =
      ...
      if (option?.cache) {
        ...
        if (!self.isNull(self.calendarApi())) {
          if (!option?.isFilter) {
            ...
            if (self.defaultView == self.viewType.month || option?.viewType) {
      ...
      if (self.defaultView == self.viewType.month) {
        ...
        if (
          ...
          Object.keys(self.studioEventsMap).forEach((key) => {
            let item = self.studioEventsMap[key];
            if (
              (item.startTime >= startUnix && item.startTime < endUnix) ||
              (item.endTime > startUnix && item.endTime <= endUnix) ||
              item.endTime == 0
            ) {
              showAllDaySlot = true;
            }
